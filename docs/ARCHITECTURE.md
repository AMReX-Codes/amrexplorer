...
| `DerivedFieldController` | one window's view of `DerivedFieldStore` -- the derived-field definitions every window of the process shares, held for the session and never persisted -- plus the Data menu's Expression Editor, its dialog and the expression-list JSON; checks a list for what is wrong whatever the data, then commits it to the store, from which every window reloads | `test_derived_field_controller`, the `--derived-field-*-smoke-test` harnesses |

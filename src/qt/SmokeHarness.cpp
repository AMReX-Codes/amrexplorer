...
// The offscreen smoke-test harnesses, moved out of main() as they were: each
// branch below arms connections and timers on the window and the application
// for one --*-smoke-test / --*-repro option, and main() then runs exec().
// They drive the window through its ForTest accessors, so they compile only
// where those do (AMREXPLORER_BUILD_TESTS), and the release binary carries
// neither.

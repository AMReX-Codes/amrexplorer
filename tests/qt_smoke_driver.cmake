...
if(MODE STREQUAL "companion-navigation")
    run_or_die("${MATERIALIZER}" "${SOURCE}" "${WORK}/upper")
    run_or_die("${MATERIALIZER}" "${SOURCE2}" "${WORK}/lower")
    run_or_die("${AMREXPLORER_QT}" --companion-navigation-smoke-test
        "${WORK}/upper" "${WORK}/lower")
elseif(MODE STREQUAL "remote-navigation" OR MODE STREQUAL "mapped-navigation")
    run_or_die("${MATERIALIZER}" "${SOURCE}" "${WORK}/plt")
    run_or_die("${AMREXPLORER_QT}" "--${MODE}-smoke-test" "${WORK}/plt")
elseif(MODE STREQUAL "navigation")
    run_or_die("${MATERIALIZER}" "${SOURCE}" "${WORK}/plt")
    run_or_die("${AMREXPLORER_QT}" --navigation-smoke-test "${WORK}/plt")
elseif(MODE STREQUAL "adaptive-precision")
    run_or_die("${MATERIALIZER}" "${SOURCE}" "${WORK}/plt")
...
elseif(MODE STREQUAL "panel-layout")
    run_or_die("${MATERIALIZER}" "${SOURCE}" "${WORK}/plt00000")
    run_or_die("${MATERIALIZER}" "${SOURCE}" "${WORK}/plt00010" "2.5")
    run_or_die("${MATERIALIZER}" "${SOURCE2}" "${WORK}/plt2d")
    run_or_die("${AMREXPLORER_QT}" --panel-layout-smoke-test
        "${WORK}/plt00000" "${WORK}/plt00010" "${WORK}/plt2d")
